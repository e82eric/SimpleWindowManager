...
int list_processes_run(CHAR** linesToFill, BOOL sort, int (*sortFunc)(const void *, const void*))
{
...
int list_processes_run_no_sort(CHAR** linesToFill)
{
    int numberOfResults = list_processes_run(linesToFill, FALSE, NULL);
    return numberOfResults;
}

int list_processes_run_sorted_by_cpu(CHAR **linesToFill)
{
    int numberOfResults = list_processes_run(linesToFill, TRUE, CompareProcessCpu);
    return numberOfResults;
}

int list_processes_run_sorted_by_private_bytes(CHAR **linesToFill)
{
    int numberOfResults = list_processes_run(linesToFill, TRUE, CompareProcessPrivateBytes);
    return numberOfResults;
}

int list_processes_run_sorted_by_working_set(CHAR **linesToFill)
{
    int numberOfResults = list_processes_run(linesToFill, TRUE, CompareProcessWorkingSet);
    return numberOfResults;
}

int list_processes_run_sorted_by_pid(CHAR **linesToFill)
{
    int numberOfResults = list_processes_run(linesToFill, TRUE, CompareProcessPid);
    return numberOfResults;
}
